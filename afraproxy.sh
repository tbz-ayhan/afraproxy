...
function RestartService() {
	pid=$(systemctl show --property MainPID afraproxy)
	arrPID=(${pid//=/ })
	pid=${arrPID[1]}
	kill -USR2 "$pid"
}
function PrintErrorJson() {
	echo "{\"status\":false,\"msg\":\"$1\"}"
	exit 1
}
function PrintOkJson() {
	echo "{\"status\":true,\"msg\":\"$1\"}"
}
...
if [ -d "/opt/afraproxy" ]; then
	SCRIPT_LOCATION=$(realpath "$0")
	cd /opt/afraproxy/ || exit 2
	if [ "$#" -ge 1 ]; then
		OPTION=$1
		if [ "$OPTION" == "list" ]; then
			...
			RestartService
			echo "{\"status\":true,\"msg\":'Restart Success'}"
			
		fi
	else
		echo "You have already installed afraproxy! What do you want to do?"
	...
	2)
		...
		if [[ "$OPTION" == "y" ]]; then
			systemctl stop afraproxy
			mv /opt/afraproxy/config.py /tmp/config.py
			git pull
			mv /tmp/config.py /opt/afraproxy/config.py
			#Update cryptography and uvloop
			pip3.8 install --upgrade cryptography uvloop
			systemctl start afraproxy
			echo "Proxy updated."
		fi
		;;
	#Change AD_TAG
	...
	#New secret
	4)
		#API Usage: bash afraproxy.sh 4 <USERNAME> <SECRET> -> Do not define secret to generate a random secret
		...
		s=$(python3.8 -c "print(\"ee\" + \"$SECRET\" + \"$TLS_DOMAIN\".encode().hex())")
		if [ "$#" -ge 2 ]; then
			echo "{\"status\":true,\"msg\":{\"link\":\"tg://proxy?server=$PUBLIC_IP&port=$PORT&secret=$s\",\"secret\":\"$SECRET\"}}"
		else
			echo
	...
	#Revoke secret
	5)
		#API usage: bash afraproxy.sh 5 <USERNAME>
		if [ "$#" -ge 2 ]; then #Get username
	...
	#Connection limits
	6)
		#API Usage: bash afraproxy.sh 6 <USERNAME> <LIMIT> -> Pass zero for unlimited
		if [ "$#" -ge 3 ]; then
			GetSecretFromUsername "$2"
		else
			ListUsersAndSelect
	...
	#Expiry date
	7)
		#API Usage: bash afraproxy.sh 7 <USERNAME> <DATE> -> Pass nothing as <DATE> to remove; Date format is dd/mm/yyyy
		if [ "$#" -ge 2 ]; then
			GetSecretFromUsername "$2"
			DATE="$3"
		else
	...
	#Quota limit stuff
	8)
		#API Usage: bash afraproxy.sh 8 <USERNAME> <LIMIT> -> Pass nothing as <LIMIT> to remove; The number is in bytes
		if [ "$#" -ge 2 ]; then
			GetSecretFromUsername "$2"
			LIMIT="$3"
		else
	...
	10)
		...
		"y")
			PORT=$(python3.8 -c 'import config;print(getattr(config, "PORT",-1))')
			systemctl stop afraproxy
			systemctl disable afraproxy
			rm -rf /opt/afraproxy /etc/systemd/system/afraproxy.service
	...
	# About
	11)
		echo "afraproxy script by Ayhan"
		echo "Source at https://github.com/tbz-ayhan/afraproxy"
		echo "Github repo of script: https://github.com/tbz-ayhan/afraproxy"
		;;
	esac
	exit
fi
...
TAG=""
COUNTER=1
echo "Welcome to Afra-Proxy auto installer!"
echo "Created by Ayhan"
echo "I will install afraproxy python script by alexbers"
echo "Source at https://github.com/tbz-ayhan/afraproxy"
echo "Github repo of script: https://github.com/tbz-ayhan/afraproxy"
...
cd /opt || exit 2
git clone https://github.com/tbz-ayhan/afraproxy.git
cd afraproxy || exit 2
...
[Service]
Type = simple
ExecStart = /usr/bin/python3.8 /opt/afraproxy/afraproxy.py
StartLimitBurst=0

[Install]
WantedBy = multi-user.target" >afraproxy.service
systemctl daemon-reload
systemctl enable afraproxy
systemctl start afraproxy
tput setaf 3
printf "%$(tput cols)s" | tr ' ' '#'
tput sgr 0
echo "Ok it must be done. I created a service to run or stop the proxy."
echo 'Use "systemctl start afraproxy" or "systemctl stop afraproxy" to start or stop it'
echo
